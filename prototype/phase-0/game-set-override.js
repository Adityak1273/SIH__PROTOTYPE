...
(() => {
  ...
  function patchHostUi(){
    ...
    const footer=document.querySelector('footer');if(footer)footer.textContent='5 guided cognitive games · adaptive training · Mimo companion · NER care extensions · training only';
  }
  ...
  function updateHeader(meta){
    if($('#gameCategory'))$('#gameCategory').textContent=meta.category;if($('#gameTitle'))$('#gameTitle').textContent=meta.title;if($('#gameCounter'))$('#gameCounter').textContent=`${state.index+1} of 5`;if($('#gamePrompt'))$('#gamePrompt').textContent=meta.description;if($('#gameMimoText'))$('#gameMimoText').textContent='Mimo will stay quiet while you play. Say “Mimo” if you need help.';const p=$('#progressBar');if(p)p.style.width=`${(state.index/5)*100}%`;
  }
