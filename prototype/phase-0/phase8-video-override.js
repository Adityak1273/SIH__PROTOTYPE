...
(() => {
  ...
  function loadGame(){ const key=state.order[state.index], g=GAMES[key], n=level(key); state.current={key,level:n,correct:0,time:0,live:false}; state.round=0; $('#gameCategory').textContent=g.area; $('#gameTitle').textContent=g.title; $('#gameCounter').textContent=`${state.index+1} of 4`; $('#gamePrompt').textContent=g.intro; $('#gameMimoText').textContent=`${label(n)} · Mimo will guide you step by step.`; $('#gameFeedback').textContent=''; const b=$('#progressBar'); if(b)b.style.width=`${state.index/4*100}%`; intro(g,n); speak(g.voice); }
